...
import {
  collection, doc, getDocs, addDoc, updateDoc, onSnapshot, query, where,
...
const iBtn  = (filled, color) => ({ background:filled?(color||C.forest):"transparent", color:filled?C.white:(color||C.forest), border:filled?"none":`1.5px solid ${color||C.forest}`, borderRadius:100, padding:"9px 18px", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" });
...
const Badge   = ({ color, bg, children }) => <span style={{ background:bg, color, borderRadius:100, padding:"2px 9px", fontSize:11, fontWeight:700, whiteSpace:"nowrap" }}>{children}</span>;
const Tag     = ({ bg="#e8f5ec", color="#2d6b45", children }) => <span style={{ background:bg, color, borderRadius:8, padding:"3px 9px", fontSize:11, fontWeight:500, whiteSpace:"nowrap" }}>{children}</span>;
const Stars   = ({ r }) => <span style={{ fontSize:12, color:C.amber }}>{"★".repeat(Math.floor(r))}<span style={{ color:"#ddd" }}>{"★".repeat(5-Math.floor(r))}</span><span style={{ color:C.muted, marginLeft:4 }}>{r}</span></span>;
const BackBtn = ({ onClick }) => <button onClick={onClick} style={{ alignSelf:"flex-start", background:C.pale, border:"none", borderRadius:100, padding:"7px 14px", fontWeight:600, fontSize:13, cursor:"pointer", color:C.forest, fontFamily:"inherit" }}>← Voltar</button>;

/* ── LOADING ─────────────────────────────────────────────────────────────── */
function LoadingScreen({ message="A carregar..." }) {
  return (
    <div style={{ minHeight:"100dvh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", background:C.cream, fontFamily:"'Segoe UI',system-ui,sans-serif" }}>
      <div style={{ width:56, height:56, borderRadius:"50% 50% 50% 14%", background:C.forest, display:"flex", alignItems:"center", justifyContent:"center", fontSize:28, marginBottom:20, animation:"pulse 1.4s ease-in-out infinite" }}>🐾</div>
      <p style={{ color:C.muted, fontSize:14, fontWeight:600 }}>{message}</p>
...
function LoginScreen({ onLogin }) {
  ...
  const handle = async (em, pw="1234") => {
    setLoading(true); setError("");
    try { await onLogin(em, pw); }
    catch(e) {
      const msgs = { "auth/user-not-found":"Utilizador não encontrado.", "auth/wrong-password":"Password incorreta.", "auth/invalid-credential":"Email ou password incorretos.", "auth/too-many-requests":"Demasiadas tentativas. Tenta mais tarde." };
      setError(msgs[e.code]||"Erro ao entrar. Tenta novamente.");
      setLoading(false);
    }
  };
      ...
      <div style={{ width:"100%", maxWidth:400 }}>
        ...
        {mode==="demo" && (
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            ...
            {demos.map((d,i)=>(
              <button key={i} onClick={()=>handle(d.email)} disabled={loading}
                style={{ background:C.white, border:`1.5px solid ${d.role==="dono"?C.border:C.purple+"55"}`, borderRadius:14, padding:"13px 16px", cursor:"pointer", fontFamily:"inherit", textAlign:"left", display:"flex", alignItems:"center", gap:12, opacity:loading?.6:1 }}
                ...
                {loading ? <span style={{ fontSize:13,color:C.muted }}>...</span> : <span style={{ color:C.muted,fontSize:18 }}>›</span>}
              </button>
            ))}
            {error && <div style={{ background:C.redBg, color:C.red, borderRadius:10, padding:"9px 14px", fontSize:13, fontWeight:600 }}>⚠️ {error}</div>}
            <p style={{ textAlign:"center", color:C.muted, fontSize:11, marginTop:4 }}>Todos os perfis usam password: <b>1234</b></p>
          </div>
        )}

        {mode==="login" && (
          <div style={{ ...card, padding:24, display:"flex", flexDirection:"column", gap:14 }}>
            <div><label style={LBL}>Email</label><input style={INPUT} type="email" placeholder="o teu email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle(email,password)} autoFocus/></div>
            <div><label style={LBL}>Password</label><input style={INPUT} type="password" placeholder="password" value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle(email,password)}/></div>
            {error && <div style={{ background:C.redBg, color:C.red, borderRadius:10, padding:"9px 14px", fontSize:13, fontWeight:600 }}>⚠️ {error}</div>}
            <button onClick={()=>handle(email,password)} disabled={loading} style={{ ...iBtn(true), width:"100%", textAlign:"center", padding:"13px 0", fontSize:14, opacity:loading?.7:1 }}>{loading?"A entrar...":"Entrar →"}</button>
          </div>
        )}
      </div>
    </div>
  ...
  if (view==="sos") return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      ...
      {done ? (
        ...
        <button onClick={submit} disabled={saving} style={{ ...iBtn(true), background:C.red, width:"100%", textAlign:"center", padding:"14px 0", fontSize:14, opacity:(!form.name||!form.area||!form.contact||saving)?.5:1 }}>
          {saving?"A publicar...":"🚨 Publicar alerta"}
        </button>
      </>}
    </div>
  );
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        ...
        <button onClick={()=>setView("sos")} style={{ background:C.red, color:C.white, border:"none", borderRadius:100, padding:"9px 16px", fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>🚨 SOS Pet</button>
      </div>
      {alerts.map(a=>(
        <div key={a.id} style={{ ...card, display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:6, flexShrink:0 }}>
            <button style={{ width:36,height:36,borderRadius:"50%",background:C.forest,border:"none",fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}>📞</button>
            <button style={{ width:36,height:36,borderRadius:"50%",background:"transparent",border:`1.5px solid ${C.forest}`,fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center" }}>🔁</button>
          </div>
        </div>
      ))}
    </div>
...
function CaregiversScreen({ user, ownerPets, onAddBooking, appData }) {
  ...
  const submitBooking = async () => {
    ...
    setSaving(true);
    await onAddBooking({ owner_id:user.id, caregiver_id:sel, pet_id:parseInt(bf.pet_id), service:bf.service, date:bf.date, time:bf.time, duration:1, status:"pendente", price:cgP.price_per_hour, notes:bf.notes, created_at:new Date().toISOString().split("T")[0] });
      ...
      {showBook ? (
        bookDone ? (
          ...
          <div style={{ ...card,border:`2px solid ${C.mint}` }}>
            ...
            <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
              <div><label style={LBL}>Animal *</label><select style={INPUT} value={bf.pet_id} onChange={e=>setBf(f=>({...f,pet_id:e.target.value}))}><option value="">Seleciona o animal</option>{ownerPets.map(p=><option key={p.id} value={p.id}>{p.species} {p.name}</option>)}</select></div>
              <div><label style={LBL}>Serviço *</label><select style={INPUT} value={bf.service} onChange={e=>setBf(f=>({...f,service:e.target.value}))}><option value="">Seleciona o serviço</option>{cgP.services.map(sv=><option key={sv} value={sv}>{sv}</option>)}</select></div>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
              ...
              <div><label style={LBL}>Notas</label><textarea style={{ ...INPUT,resize:"none",height:60 }} placeholder="Instruções especiais..." value={bf.notes} onChange={e=>setBf(f=>({...f,notes:e.target.value}))}/></div>
            </div>
            <div style={{ display:"flex",gap:8,marginTop:12 }}>
              <button onClick={submitBooking} disabled={saving} style={{ ...iBtn(true),flex:1,textAlign:"center",padding:"12px 0",opacity:(!bf.pet_id||!bf.service||!bf.date||!bf.time||saving)?.5:1 }}>{saving?"A enviar...":"Enviar pedido"}</button>
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
      ...
      <div style={{ display:"flex",alignItems:"center",gap:8,background:C.white,border:`1px solid ${C.border}`,borderRadius:100,padding:"9px 14px" }}><span>🔍</span><span style={{ color:"#bbb",fontSize:13 }}>Pesquisar cuidadores...</span></div>
      <div style={{ display:"flex",gap:6,overflowX:"auto",paddingBottom:2 }}>
        {services.map(sv=><button key={sv} onClick={()=>setFilter(sv)} style={{ background:filter===sv?C.forest:"#f0f7f2",color:filter===sv?C.white:C.muted,border:"none",borderRadius:100,padding:"6px 14px",fontSize:12,fontWeight:600,cursor:"pointer",whiteSpace:"nowrap",fontFamily:"inherit" }}>{sv}</button>)}
      </div>
      {list.map(c=>{ const u=users.find(us=>us.id===c.user_id); return (
        <div key={c.user_id} onClick={()=>setSel(c.user_id)} style={{ ...card,display:"flex",alignItems:"center",gap:12,cursor:"pointer" }}>
          ...
          <div style={{ flex:1,minWidth:0 }}><div style={{ display:"flex",alignItems:"center",gap:6 }}><span style={{ fontWeight:700,fontSize:15 }}>{u?.name}</span>{c.verified&&<span>✅</span>}</div><Stars r={c.rating}/><div style={{ display:"flex",gap:6,marginTop:4,flexWrap:"wrap" }}>{c.services.slice(0,2).map(sv=><Tag key={sv}>{sv}</Tag>)}</div></div>
          <div style={{ textAlign:"right",flexShrink:0 }}><div style={{ fontWeight:700,color:C.forest,fontSize:15 }}>{c.price_per_hour}€/h</div><div style={{ fontSize:12,color:"#bbb" }}>{c.distance}</div><Badge color={c.available?"#16773a":"#888"} bg={c.available?"#dcf0e3":"#f0f4f8"}>{c.available?"Disponível":"Ocupado"}</Badge></div>
        </div>
      ); })}
    </div>
  );
...
/* ── DONO: BOOKINGS ──────────────────────────────────────────────────────── */
function DonoBookingsScreen({ user, bookings, onUpdateBooking, appData }) {
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
      ...
      {mine.length===0&&<div style={{ ...card,textAlign:"center",padding:40,color:C.muted }}><div style={{ fontSize:40,marginBottom:12 }}>🗓️</div><div style={{ fontWeight:600 }}>Sem reservas ainda</div><div style={{ fontSize:13,marginTop:6 }}>Vai a Cuidadores para fazer a primeira reserva!</div></div>}
            ...
            {list.map(b=>{ const cU=users.find(u=>u.id===b.caregiver_id); const pet=pets.find(p=>p.id===b.pet_id); return (
              <div key={b.id} style={{ ...card,marginBottom:10,border:status==="orçamento_enviado"?`2px solid ${C.purple}`:`1px solid ${C.border}` }}>
                <div style={{ display:"flex",alignItems:"center",gap:12,marginBottom:b.quote_price?12:0 }}>
                  ...
                  <div style={{ flex:1,minWidth:0 }}><div style={{ fontWeight:700,fontSize:14 }}>{pet?.species} {pet?.name} com {cU?.name}</div><div style={{ color:C.muted,fontSize:13,marginTop:2 }}>{b.service} · {b.date} {b.time}</div>{b.notes&&<div style={{ fontSize:12,color:C.muted,fontStyle:"italic",marginTop:2 }}>"{b.notes}"</div>}</div>
                  <div style={{ textAlign:"right",flexShrink:0 }}><div style={{ fontWeight:700,color:C.forest,fontSize:15,marginBottom:4 }}>{b.quote_price||b.price}€</div><Badge color={STATUS_COLORS[b.status]?.[0]} bg={STATUS_COLORS[b.status]?.[1]}>{b.status}</Badge></div>
                </div>
                    ...
                    <div style={{ background:"#ede8fc",borderRadius:12,padding:14,marginBottom:12,border:"1px solid #c8b8f8" }}>
                      ...
                      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:b.quote_notes?10:0 }}>
                        {[["Valor",`${b.quote_price}€`],["Data",b.quote_date||b.date],["Hora",b.quote_time||b.time],["Cuidador",cU?.name]].map(([k,v])=>(
                          <div key={k} style={{ background:"rgba(255,255,255,0.7)",borderRadius:9,padding:"8px 10px" }}><div style={{ fontSize:10,color:C.purple,fontWeight:700,textTransform:"uppercase" }}>{k}</div><div style={{ fontWeight:700,fontSize:14,marginTop:2,color:C.purple }}>{v}</div></div>
                        ))}
                      </div>
                      {b.quote_notes&&<div style={{ fontSize:13,color:"#6c5ce7",fontStyle:"italic" }}>📝 "{b.quote_notes}"</div>}
                    </div>
                    <div style={{ display:"flex",gap:8 }}>
                      <button onClick={()=>onUpdateBooking(b.id,{status:"confirmado",price:b.quote_price,date:b.quote_date||b.date,time:b.quote_time||b.time})} style={{ ...iBtn(true),flex:1,textAlign:"center",padding:"12px 0" }}>✅ Aceitar orçamento</button>
                      <button onClick={()=>onUpdateBooking(b.id,{status:"rejeitado"})} style={{ ...iBtn(true),background:C.red,padding:"12px 16px" }}>❌</button>
                    </div>
                  </>
                )}
              </div>
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8 }}>
        {[{label:"Pets",icon:"🐾",val:ownerPets.length,tab:"pets",bg:"#e8f5ec"},{label:"Alertas",icon:"📍",val:alerts.length,tab:"alerts",bg:"#fff3e0"},{label:"Reservas",icon:"🗓️",val:mine.length,tab:"bookings",bg:"#ede8fc"}].map(s=>(
          <div key={s.tab} onClick={()=>setTab(s.tab)} style={{ ...card,background:s.bg,border:"none",cursor:"pointer",padding:"14px 12px",textAlign:"center" }}><div style={{ fontSize:22 }}>{s.icon}</div><div style={{ fontWeight:800,fontSize:22,margin:"4px 0 0" }}>{s.val}</div><div style={{ fontSize:11,color:C.muted,fontWeight:600 }}>{s.label}</div></div>
        ))}
      </div>
      {active&&<div style={{ background:`linear-gradient(135deg,${C.forest},#2d7a48)`,borderRadius:16,padding:16,color:"#fff",display:"flex",alignItems:"center",gap:12 }}><div style={{ width:44,height:44,borderRadius:"50%",background:"rgba(255,255,255,.15)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22 }}>{users.find(u=>u.id===active.caregiver_id)?.avatar}</div><div style={{ flex:1 }}><div style={{ fontSize:11,opacity:.7 }}>A decorrer agora</div><div style={{ fontWeight:700,fontSize:15 }}>{pets.find(p=>p.id===active.pet_id)?.name} com {users.find(u=>u.id===active.caregiver_id)?.name}</div><div style={{ fontSize:12,opacity:.8 }}>{active.service} · {active.date}</div></div></div>}
      {quotePending>0&&<div onClick={()=>setTab("bookings")} style={{ ...card,background:"#ede8fc",border:"2px solid #c8b8f8",cursor:"pointer",display:"flex",alignItems:"center",gap:12 }}><span style={{ fontSize:24 }}>💶</span><div style={{ flex:1 }}><div style={{ fontWeight:700,fontSize:14,color:C.purple }}>{quotePending} orçamento{quotePending>1?"s":""} a aguardar a tua resposta</div><div style={{ fontSize:13,color:"#9b8fd4" }}>Toca para ver e aceitar</div></div><span style={{ fontSize:18,color:C.purple }}>›</span></div>}
      {pending>0&&<div onClick={()=>setTab("bookings")} style={{ ...card,background:"#fff3e0",border:"1px solid #f5d08a",cursor:"pointer",display:"flex",alignItems:"center",gap:12 }}><span style={{ fontSize:24 }}>⏳</span><div style={{ flex:1 }}><div style={{ fontWeight:700,fontSize:14 }}>{pending} reserva{pending>1?"s":""} a aguardar confirmação</div><div style={{ fontSize:13,color:C.muted }}>Toca para ver detalhes</div></div><span style={{ fontSize:18,color:C.muted }}>›</span></div>}
      <div>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10 }}><span style={{ fontWeight:700,fontSize:15 }}>Os meus animais</span><span onClick={()=>setTab("pets")} style={{ color:C.moss,fontSize:13,fontWeight:600,cursor:"pointer" }}>Ver todos →</span></div>
        <div style={{ display:"flex",gap:10,overflowX:"auto",paddingBottom:4 }}>
          {ownerPets.map(p=><div key={p.id} onClick={()=>setTab("pets")} style={{ ...card,minWidth:130,flexShrink:0,textAlign:"center",background:p.color,border:"none",cursor:"pointer" }}><div style={{ fontSize:30,marginBottom:6 }}>{p.species}</div><div style={{ fontWeight:700,fontSize:14 }}>{p.name}</div><div style={{ fontSize:11,color:C.muted,margin:"2px 0 8px" }}>{p.breed}</div><Badge color={p.status==="Em casa"?"#16773a":"#c47a00"} bg={p.status==="Em casa"?"#dcf0e3":"#fff3e0"}>{p.status}</Badge></div>)}
        </div>
      </div>
    </div>
  );
...
function CuidadorDashboard({ user, profile, bookings, setTab, appData }) {
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}><div><p style={{ color:C.muted,fontSize:13,margin:0 }}>Painel do cuidador</p><h1 style={{ margin:0,fontSize:20,fontWeight:800 }}>Olá, {user.name.split(" ")[0]} {user.avatar}</h1></div><Badge color={profile?.available?"#16773a":"#888"} bg={profile?.available?"#dcf0e3":"#f0f4f8"}>{profile?.available?"● Disponível":"● Ocupado"}</Badge></div>
      <div style={{ display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8 }}>
        {[{label:"Pedidos",icon:"📩",val:pending.length,tab:"requests",bg:"#fff3e0"},{label:"Confirmados",icon:"✅",val:mine.filter(b=>b.status==="confirmado").length,tab:"requests",bg:"#e8f5ec"},{label:"Ganhos €",icon:"💶",val:earning,tab:"requests",bg:"#ede8fc"}].map(s=>(
          <div key={s.label} onClick={()=>setTab(s.tab)} style={{ ...card,background:s.bg,border:"none",cursor:"pointer",padding:"14px 12px",textAlign:"center" }}><div style={{ fontSize:22 }}>{s.icon}</div><div style={{ fontWeight:800,fontSize:22,margin:"4px 0 0" }}>{s.val}{s.label==="Ganhos €"?"€":""}</div><div style={{ fontSize:11,color:C.muted,fontWeight:600 }}>{s.label}</div></div>
        ))}
      </div>
      {pending.length>0&&<div style={{ ...card,background:"#fff3e0",border:"1px solid #f5d08a" }}><div style={{ fontWeight:700,fontSize:15,marginBottom:12 }}>📩 Novos pedidos ({pending.length})</div>{pending.slice(0,2).map(b=>{ const owner=users.find(u=>u.id===b.owner_id); const pet=pets.find(p=>p.id===b.pet_id); return <div key={b.id} style={{ display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderTop:`1px solid #f5d08a` }}><span style={{ fontSize:22 }}>{owner?.avatar}</span><div style={{ flex:1 }}><div style={{ fontWeight:600,fontSize:14 }}>{owner?.name}</div><div style={{ fontSize:12,color:C.muted }}>{pet?.species} {pet?.name} · {b.service} · {b.date}</div></div><button onClick={()=>setTab("requests")} style={{ ...iBtn(true),padding:"6px 12px",fontSize:12 }}>Ver</button></div>; })}{pending.length>2&&<div onClick={()=>setTab("requests")} style={{ textAlign:"center",color:C.moss,fontSize:13,fontWeight:600,cursor:"pointer",paddingTop:8 }}>+{pending.length-2} mais →</div>}</div>}
      <div style={card}><div style={{ fontWeight:700,fontSize:15,marginBottom:12 }}>📊 Resumo geral</div><div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>{[["Total reservas",mine.length],["Avaliação",profile?.rating||"—"],["Avaliações",profile?.reviews||0],["Preço/h",`${profile?.price_per_hour||0}€`]].map(([k,v])=><div key={k} style={{ background:C.pale,borderRadius:12,padding:12 }}><div style={{ fontSize:11,color:C.muted,fontWeight:700,textTransform:"uppercase" }}>{k}</div><div style={{ fontWeight:800,fontSize:20,color:C.forest,marginTop:4 }}>{v}</div></div>)}</div></div>
    </div>
  );
}

/* ── CUIDADOR: AGENDA ────────────────────────────────────────────────────── */
function CuidadorAgenda({ user, bookings, onUpdateBooking, appData }) {
  const { users, pets } = appData;
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
      ...
      {mine.length===0&&<div style={{ ...card,textAlign:"center",padding:40,color:C.muted }}><div style={{ fontSize:40,marginBottom:12 }}>🗓️</div><div style={{ fontWeight:600 }}>Sem serviços confirmados</div><div style={{ fontSize:13,marginTop:6 }}>Os pedidos aceites aparecem aqui.</div></div>}
          ...
          <div key={status}>
            ...
            {list.map(b=>{ const owner=users.find(u=>u.id===b.owner_id); const pet=pets.find(p=>p.id===b.pet_id); return (
              <div key={b.id} style={{ ...card,marginBottom:10 }}>
                <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:10 }}><div style={{ width:40,height:40,borderRadius:"50%",background:`linear-gradient(135deg,${C.mint},${C.moss})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0 }}>{owner?.avatar}</div><div style={{ flex:1 }}><div style={{ fontWeight:700,fontSize:14 }}>{owner?.name}</div><div style={{ color:C.muted,fontSize:12 }}>{owner?.phone}</div></div><Badge color={STATUS_COLORS[b.status]?.[0]} bg={STATUS_COLORS[b.status]?.[1]}>{b.status}</Badge></div>
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginBottom:10 }}>{[["Animal",`${pet?.species} ${pet?.name}`],["Serviço",b.service],["Data",`${b.date} ${b.time}`],["Valor",`${b.quote_price||b.price}€`]].map(([k,v])=><div key={k} style={{ background:C.pale,borderRadius:10,padding:"8px 10px" }}><div style={{ fontSize:10,color:C.muted,fontWeight:700,textTransform:"uppercase" }}>{k}</div><div style={{ fontWeight:600,fontSize:13,marginTop:2 }}>{v}</div></div>)}</div>
                ...
                {status==="confirmado"&&<button onClick={()=>onUpdateBooking(b.id,{status:"a decorrer"})} style={{ ...iBtn(true),width:"100%",textAlign:"center",padding:"10px 0",background:C.amber }}>▶ Iniciar serviço</button>}
                {status==="a decorrer"&&<button onClick={()=>onUpdateBooking(b.id,{status:"concluido"})} style={{ ...iBtn(false),width:"100%",textAlign:"center",padding:"10px 0" }}>✓ Concluir serviço</button>}
              </div>
            ); })}
          </div>
        );
...
/* ── CUIDADOR: PEDIDOS ───────────────────────────────────────────────────── */
function CuidadorPedidos({ user, bookings, onUpdateBooking, appData }) {
  const { users, pets } = appData;
  ...
  const sendQuote=async(b)=>{
    if(!qForm.price) return;
    setSaving(true);
    await onUpdateBooking(b.id,{status:"orçamento_enviado",quote_price:parseFloat(qForm.price),quote_notes:qForm.notes,quote_date:qForm.visit_date||b.date,quote_time:qForm.visit_time||b.time});
    setSaving(false); setQuoteOpen(null); setQForm({price:"",notes:"",visit_date:"",visit_time:""});
  };

  return (
    <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
      <div><h2 style={{ margin:0,fontSize:20,fontWeight:800 }}>📩 Pedidos</h2><p style={{ color:C.muted,fontSize:13,margin:"4px 0 0" }}>Analisa cada pedido e envia um orçamento ao dono</p></div>
      {mine.length===0&&<div style={{ ...card,textAlign:"center",padding:40,color:C.muted }}><div style={{ fontSize:40,marginBottom:12 }}>📭</div><div style={{ fontWeight:600 }}>Sem pedidos de momento</div></div>}
      {["pendente","orçamento_enviado","rejeitado"].map(status=>{
        ...
        return (
          <div key={status}>
            <div style={{ fontSize:11,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:".06em",marginBottom:8 }}>{labels[status]} ({list.length})</div>
            {list.map(b=>{ const owner=users.find(u=>u.id===b.owner_id); const pet=pets.find(p=>p.id===b.pet_id); const isOpen=quoteOpen===b.id; return (
              <div key={b.id} style={{ ...card,marginBottom:10,border:status==="pendente"?`2px solid ${C.mint}`:`1px solid ${C.border}` }}>
                <div style={{ display:"flex",alignItems:"center",gap:10,marginBottom:12 }}><div style={{ width:42,height:42,borderRadius:"50%",background:`linear-gradient(135deg,${C.mint},${C.moss})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,flexShrink:0 }}>{owner?.avatar}</div><div style={{ flex:1 }}><div style={{ fontWeight:700,fontSize:14 }}>{owner?.name}</div><div style={{ color:C.muted,fontSize:12 }}>📞 {owner?.phone}</div></div><Badge color={STATUS_COLORS[b.status]?.[0]} bg={STATUS_COLORS[b.status]?.[1]}>{b.status}</Badge></div>
                <div style={{ background:C.pale,borderRadius:12,padding:12,marginBottom:10 }}>
                  ...
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>{[["Animal",`${pet?.species} ${pet?.name}`],["Serviço",b.service],["Data pedida",b.date],["Hora pedida",b.time]].map(([k,v])=><div key={k}><div style={{ fontSize:10,color:C.muted,fontWeight:700,textTransform:"uppercase" }}>{k}</div><div style={{ fontWeight:600,fontSize:13,marginTop:2 }}>{v}</div></div>)}</div>
                  {b.notes&&<div style={{ marginTop:8,fontSize:13,color:C.muted,fontStyle:"italic" }}>📝 "{b.notes}"</div>}
                </div>
                {b.quote_price&&<div style={{ background:"#ede8fc",borderRadius:12,padding:12,marginBottom:10,border:"1px solid #c8b8f8" }}><div style={{ fontSize:11,color:C.purple,fontWeight:700,textTransform:"uppercase",marginBottom:8 }}>Orçamento enviado</div><div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6 }}>{[["Valor",`${b.quote_price}€`],["Data",`${b.quote_date||b.date} ${b.quote_time||b.time}`]].map(([k,v])=><div key={k}><div style={{ fontSize:10,color:C.purple,fontWeight:700,textTransform:"uppercase" }}>{k}</div><div style={{ fontWeight:700,fontSize:14,marginTop:2,color:C.purple }}>{v}</div></div>)}</div>{b.quote_notes&&<div style={{ marginTop:8,fontSize:13,color:"#6c5ce7",fontStyle:"italic" }}>📝 "{b.quote_notes}"</div>}</div>}
                {status==="pendente"&&(isOpen?(
                  <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
                    ...
                    <div style={{ display:"flex",gap:8 }}>
                      <button onClick={()=>sendQuote(b)} disabled={saving} style={{ ...iBtn(true),flex:1,textAlign:"center",padding:"11px 0",opacity:(!qForm.price||saving)?.5:1 }}>{saving?"A enviar...":"📤 Enviar orçamento"}</button>
                    ...
                    <button onClick={()=>{setQuoteOpen(null);onUpdateBooking(b.id,{status:"rejeitado"});}} style={{ ...iBtn(true),background:C.red,width:"100%",textAlign:"center",padding:"10px 0" }}>❌ Recusar pedido</button>
                  </div>
                ):(
                  <div style={{ display:"flex",gap:8 }}>
                    ...
                    <button onClick={()=>onUpdateBooking(b.id,{status:"rejeitado"})} style={{ ...iBtn(true),background:C.red,padding:"11px 14px" }}>❌</button>
                  </div>
                ))}
                {status==="orçamento_enviado"&&<div style={{ display:"flex",alignItems:"center",gap:8,background:"#f8f7ff",borderRadius:10,padding:"10px 14px",fontSize:13,color:C.purple }}><span>⏳</span> A aguardar confirmação do dono...</div>}
              </div>
            ); })}
          </div>
        );
...
/* ── CUIDADOR: PERFIL ────────────────────────────────────────────────────── */
function CuidadorProfile({ user, profile, appData }) {
  const { users, reviews } = appData;
  ...
  return (
    <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
      ...
      {myReviews.length>0&&<div style={card}><div style={{ fontSize:11,color:C.muted,fontWeight:700,textTransform:"uppercase",marginBottom:12 }}>Avaliações recentes</div>{myReviews.map(r=>{ const o=users.find(u=>u.id===r.owner_id); return <div key={r.id} style={{ paddingBottom:12,borderBottom:`1px solid ${C.border}`,marginBottom:12 }}><div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:4 }}><span>{o?.avatar}</span><span style={{ fontWeight:600,fontSize:14 }}>{o?.name}</span><Stars r={r.rating}/></div><p style={{ fontSize:13,color:C.text,margin:0 }}>"{r.comment}"</p></div>; })}</div>}
    </div>
  );
}

/* ── ROOT APP ────────────────────────────────────────────────────────────── */
const DONO_TABS    = [{id:"dashboard",icon:"🏠",label:"Início"},{id:"pets",icon:"🐾",label:"Pets"},{id:"caregivers",icon:"👤",label:"Cuidadores"},{id:"alerts",icon:"📍",label:"Alertas"},{id:"bookings",icon:"🗓️",label:"Reservas"}];
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    (async()=>{
      const [uSnap,pSnap,cgSnap,rSnap] = await Promise.all([
        getDocs(collection(db,"users")),
        getDocs(collection(db,"pets")),
        getDocs(collection(db,"caregiver_profiles")),
        getDocs(collection(db,"reviews")),
      ]);
      setAppData({
        users:             uSnap.docs.map(d=>({ ...d.data(), id: isNaN(d.id)?d.id:parseInt(d.id) })),
        pets:              pSnap.docs.map(d=>({ ...d.data() })),
        caregiverProfiles: cgSnap.docs.map(d=>({ ...d.data() })),
  ...
  useEffect(()=>{
    if (!userProfile) return;
    const unsubB = onSnapshot(collection(db,"bookings"), snap=>setBookings(snap.docs.map(d=>({id:d.id,...d.data()}))));
    const unsubA = onSnapshot(collection(db,"alerts"),   snap=>setAlerts(snap.docs.map(d=>({id:d.id,...d.data()}))));
  ...
  const handleUpdateBooking = (id,fields) => updateDoc(doc(db,"bookings",id),fields);
  const handleAddAlert      = data        => addDoc(collection(db,"alerts"),data);

  if (authLoading)  return <LoadingScreen message="A verificar sessão..."/>;
  ...
  const screens = isDono ? {
    dashboard:  <DonoDashboard     user={userProfile} setTab={setTab} ownerPets={ownerPets} bookings={bookings} alerts={alerts} appData={appData}/>,
    pets:       <PetsScreen        ownerPets={ownerPets}/>,
    caregivers: <CaregiversScreen  user={userProfile} ownerPets={ownerPets} onAddBooking={handleAddBooking} appData={appData}/>,
    alerts:     <AlertsScreen      alerts={alerts} onAddAlert={handleAddAlert}/>,
    bookings:   <DonoBookingsScreen user={userProfile} bookings={bookings} onUpdateBooking={handleUpdateBooking} appData={appData}/>,
  } : {
    dashboard: <CuidadorDashboard user={userProfile} profile={cgProfile} bookings={bookings} setTab={setTab} appData={appData}/>,
    requests:  <CuidadorPedidos   user={userProfile} bookings={bookings} onUpdateBooking={handleUpdateBooking} appData={appData}/>,
    schedule:  <CuidadorAgenda    user={userProfile} bookings={bookings} onUpdateBooking={handleUpdateBooking} appData={appData}/>,
    alerts:    <AlertsScreen      alerts={alerts} onAddAlert={handleAddAlert}/>,
    profile:   <CuidadorProfile   user={userProfile} profile={cgProfile} appData={appData}/>,
  };
  ...
  const Sidebar = () => (
    <>
      ...
      <div style={{ marginTop:"auto",padding:12,background:"#f0f7f2",borderRadius:14,display:"flex",alignItems:"center",gap:10 }}>
        <div style={{ width:34,height:34,borderRadius:"50%",background:`linear-gradient(135deg,${C.mint},${C.moss})`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16 }}>{userProfile.avatar}</div>
        <div style={{ flex:1 }}><div style={{ fontWeight:700,fontSize:13 }}>{userProfile.name.split(" ")[0]}</div><div style={{ fontSize:11,color:C.muted,textTransform:"capitalize" }}>{userProfile.role}</div></div>
        <button onClick={handleLogout} title="Sair" style={{ background:"none",border:"none",cursor:"pointer",fontSize:16,color:C.muted }}>🚪</button>
      </div>
    </>
  );
  ...
  if (mobile) return (
    <div style={{ height:"100dvh",display:"flex",flexDirection:"column",background:C.cream,fontFamily:"'Segoe UI',system-ui,sans-serif",color:C.text }}>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",background:C.white,borderBottom:`1px solid ${C.border}`,flexShrink:0 }}>
        ...
        <button onClick={handleLogout} style={{ background:"none",border:"none",cursor:"pointer",fontSize:20 }}>🚪</button>
      </div>
